...
void WorldSession::HandleQuestgiverStatusQueryOpcode(WorldPacket & recv_data)
{
    ...
    switch (questgiver->GetTypeId())
    {
        case TYPEID_UNIT:
        {
            ...
            {
                questStatus = sOldScriptMgr.GetDialogStatus(_player, cr_questgiver);
                if (questStatus > 6)
        ...
        case TYPEID_GAMEOBJECT:
        {
            ...
            questStatus = sOldScriptMgr.GetDialogStatus(_player, go_questgiver);
            if (questStatus > 6)
...
void WorldSession::HandleQuestgiverHelloOpcode(WorldPacket & recv_data)
{
    ...
    if (sOldScriptMgr.OnGossipHello(_player, pCreature))
        return;
...
void WorldSession::HandleQuestgiverAcceptQuestOpcode(WorldPacket & recv_data)
{
    ...
    if (qInfo)
    {
        ...
        if (_player->CanAddQuest(qInfo, true))
        {
            ...
            {
                case TYPEID_UNIT:
                    sOldScriptMgr.OnQuestAccept(_player, ((Creature*)pObject), qInfo);
                    break;
                case TYPEID_ITEM:
                case TYPEID_CONTAINER:
                {
                    sOldScriptMgr.OnQuestAccept(_player, ((Item*)pObject), qInfo);
                ...
                case TYPEID_GAMEOBJECT:
                    sOldScriptMgr.OnQuestAccept(_player, ((GameObject*)pObject), qInfo);
                    break;
            }
            _player->PlayerTalkClass->CloseGossip();
    ...
    for (Player::ClientGUIDs::iterator itr = _player->m_clientGUIDs.begin(); itr != _player->m_clientGUIDs.end(); ++itr)
    {
        ...
        if (IS_CREATURE_GUID(*itr))
        {
            ...
            if (!questgiver->HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_QUESTGIVER))
                continue;
            questStatus = sOldScriptMgr.GetDialogStatus(_player, questgiver);
            if (questStatus > 6)
